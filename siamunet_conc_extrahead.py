...
class SiamUnet_conc(nn.Module):
    ...
    def __init__(self, input_nbr, label_nbr):
        ...
        self.cov_h1 = nn.Conv2d(label_nbr,16, kernel_size=3, padding=1, stride=4)
        self.cov_h2 = nn.Conv2d(16,8, kernel_size=3, padding=1, stride=2)
        self.fc_h1 = nn.Linear(10368 , 32)
        self.do_h1 = nn.Dropout(p=0.1)
        self.fc_h2 = nn.Linear(32, 1)
        self.fc_h3 = nn.Linear(1, 1)

        # self.sm = nn.LogSoftmax(dim=1)
        # self.fc1 = nn.Linear(8649, 1)
    ...
    def forward(self, x1, x2):
        ...
        # return self.sm(x11d)
        branch = self.cov_h1(x11d)
        branch = nn.LeakyReLU(0.5)(branch)
        branch = self.cov_h2(branch)
        branch = nn.LeakyReLU(0.5)(branch)
        branch = torch.flatten(branch,1)
        branch = F.relu(self.fc_h1(branch))
        branch = self.do_h1(branch)
        branch = nn.Sigmoid()(self.fc_h2(branch))
        out = self.fc_h3(branch)
