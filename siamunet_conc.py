...
class SiamUnet_conc(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # return self.sm(x11d)
        last_act = F.relu(x11d)
        pool_m = F.max_pool2d(last_act, kernel_size=12, stride=3)
        # pool_m = F.avg_pool2d(last_act, kernel_size=12, stride=3)
        flat = torch.flatten(pool_m, 1)
        out = self.fc1(flat)
