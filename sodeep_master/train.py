...
if __name__ == '__main__':
    ...
    parser.add_argument("-pf", dest="print_frequency", help="Number of element processed between print", type=int,
                        default=1)
    parser.add_argument("-slen", dest="seq_len", help="lenght of the sequence process by the ranker", type=int,
                        default=30)
    parser.add_argument("-d", dest="dist",
                        help="index of a single distribution for dataset if None all the distribution will be used.",
                        default=None)
    parser.add_argument('-m', dest="model_type",
                        help="Specify which model to use. (lstm, grus, gruc, grup, exa, lstmla, lstme, mlp, cnn) ",
                        default='lstmla')
    ...
    train_loader = DataLoader(dset, batch_size=args.batch_size, shuffle=False, num_workers=8,
                              sampler=SubsetRandomSampler(range(int(len(dset) * 0.1), len(dset))))
    val_loader = DataLoader(dset, batch_size=args.batch_size, shuffle=False, num_workers=8,
                            sampler=SubsetRandomSampler(range(int(len(dset) * 0.1))))
    ...
    for epoch in range(start_epoch, args.mepoch):
        is_best = False
        lr_scheduler.step()
        train_loss, batch_train, data_train = train(train_loader, model, criterion, optimizer, epoch,
        ...
        log_epoch(writer, epoch, train_loss, val_loss, optimizer.param_groups[0]['lr'], batch_train, batch_val,
                  data_train, data_val)
